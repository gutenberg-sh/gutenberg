...
import type { GutenbergManifest } from '../manifest/manifest.types';

import { MarkdownRenderService } from './markdown-render.service';

export type VerifiedSiteGatewayInput = {
  host: string;
  port: number;
  name: string;
  version: string;
...
export class LocalSiteGatewayService {
  constructor(private readonly markdownRender: MarkdownRenderService) {}

  async listen(
    input: VerifiedSiteGatewayInput,
  ...
  private handle_request(
    ...
    try {
      ...
      const mime = mime_for_path(site_path);
  ...
  private render_md_page(options: {
    ...
    const html_body =
      this.markdownRender.render_markdown_fragment(options.markdown);
    ...
    const nav_links = markdown_paths_from_manifest(options.manifest)
      .sort()
      .map((p) => {
...
function markdown_paths_from_manifest(
  manifest: GutenbergManifest,
): `/${string}`[] {
  return Object.keys(manifest.files).filter((p) =>
    p.endsWith('.md'),
  ) as `/${string}`[];
}

function mime_for_path(site_path: `/${string}`): string {
  const ext = site_path.slice(site_path.lastIndexOf('.')).toLowerCase();
...
/** Wait until the user hits Ctrl+C, then resolve. */
